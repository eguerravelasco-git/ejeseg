...
@ManagedBean
@RequestScoped
public class CtrlUrbanoRangoEdad {
    ...
    private CartesianChartModel edades(int anio) {
        ...
        try {
            ...
            for (int i = 0; i < lstRangos.size(); i++) {                
                edades.set(lstRangos.get(i).getEspecificacion(), FBVifUrbano.obtenerDatosDadoAnioEdad(anio, lstRangos.get(i).getRango()).size());
            }
    ...
    private CartesianChartModel edadesGenero(int anio) {
        ...
        try {
            ...
            for (int i = 0; i < lstRangos.size(); i++) {                
                femenino.set(lstRangos.get(i).getEspecificacion(), FBVifUrbano.obtenerDatosDadoAnioEdadGenero(anio, lstRangos.get(i).getRango(),"F").size());
            }
            
            ChartSeries masculino = new ChartSeries();
            masculino.setLabel("Masculino");            
            for (int i = 0; i < lstRangos.size(); i++) {                
                masculino.set(lstRangos.get(i).getEspecificacion(), FBVifUrbano.obtenerDatosDadoAnioEdadGenero(anio, lstRangos.get(i).getRango(),"M").size());
            }
