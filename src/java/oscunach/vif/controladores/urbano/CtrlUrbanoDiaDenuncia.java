...
@ManagedBean
@RequestScoped
public class CtrlUrbanoDiaDenuncia {
    ...
    private CartesianChartModel diasAtencion(int anio) {
        ...
        try {
            ...
            for (int i = 0; i < lst.size(); i++) {
                dias.set(lst.get(i), FBVifUrbano.obtenerDatosDadoAnioDiaDenuncia(anio, lst.get(i)).size());
            }
    ...
    private CartesianChartModel diasAtencionGenero(int anio) {
        ...
        try {
            ...
            for (int i = 0; i < lst.size(); i++) {
                femenino.set(lst.get(i), FBVifUrbano.obtenerDatosDadoAnioDiaDenunciaGenero(anio, lst.get(i), "F").size());
            }

            ChartSeries masculino = new ChartSeries();
            masculino.setLabel("Masculino");
            for (int i = 0; i < lst.size(); i++) {
                masculino.set(lst.get(i), FBVifUrbano.obtenerDatosDadoAnioDiaDenunciaGenero(anio, lst.get(i), "M").size());
            }
